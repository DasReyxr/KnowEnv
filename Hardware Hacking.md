...
Defensive Coding
7. State errors
Sign & verify analyze all state variablese
...
# 26-5-29
Pierce Gate
The pierce gate configuration is a type of oscillator circuit that uses a crystal to stabilize the frequency of oscillation. It is commonly used in digital circuits to provide a stable clock signal. The pierce gate oscillator consists of an inverter, a feedback resistor, and a crystal connected in parallel with the inverter. The crystal determines the frequency of oscillation, while the feedback resistor controls the gain of the circuit. This configuration is known for its simplicity and reliability, making it a popular choice for generating clock signals in microcontrollers and other digital devices.
![alt text](src/PierceGate.png)

# JTAG Boundary Scan
JTAG boundary scan is a method for testing the interconnections of integrated circuits (ICs), nowdays its not so used by 

```Where I work we've looked at Boundary Scan at some point, but decided against it due to the astronomical cost of tools and licenses. It may make sense for products that are high-end and high-volume though.
```
[JTAG](https://www.reddit.com/r/embedded/comments/t38ocr/jtag_for_boundary_scan/)



#
Embedded software is developed under the assumption that hardware execution is always correct. 


[Fault Attacks on Secure Embedded Software: Threats, Design and Evaluation](https://arxiv.org/pdf/2003.10513)
